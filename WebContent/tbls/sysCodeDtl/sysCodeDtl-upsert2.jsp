...
<sql:transaction>
	...
	<c:when test="${action eq 'N' or action eq 'C'}">
		<sql:update>
		insert	into sysCodeDtl (sysCode_id, cd, nm, optGroupName, sta_ymd, end_ymd)
		values
		(
				?			<sql:param value="${paramValues.sysCode_id[s.index]}"/>
...
<sql:update>
update	sysCodeDtl
   set	   sysCode_id = ?			<sql:param value="${paramValues.sysCode_id[s.index]}"/>
